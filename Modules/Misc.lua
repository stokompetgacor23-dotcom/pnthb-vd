...
function Misc.StartAutoGenerator()
    ...
    Misc.GenConnection = RunService.Heartbeat:Connect(function()
        ...
        else inside = (lr >= startPos and lr <= endPos) end
        if inside then Misc.LastSkillHit = tick(); PressSkill() end
    end)
end
...
function Misc.StartAntiStuckThread()
    task.spawn(function()
        while task.wait(0.25) do
            if not Misc.Config.Current.AutoGenerator then continue end
            local line = GetSkillCheck()
            if not line and tick() - Misc.LastSkillHit > 1.1 then
                pcall(function() Misc.Utils.ForceUnstuck() end)
            end
        end
    end)
end
...
function Misc.TriggerAntiStuck()
    pcall(function()
        ...
        if hum and root then
            ...
            hum.WalkSpeed = Misc.Config.Current.SpeedBoost and (17 + (17 * ((tonumber(Misc.Config.Current.BoostSpeed) or 0) / 100))) or 17
                ...
                if genFolder then
                    ...
                    for _, gen in ipairs(genFolder:GetChildren()) do
                        ...
                        if part then
                            ...
                            if not nearestDist or dist < nearestDist then nearestDist = dist; nearestGen = part end
                        end
                    end
        ...
        if Misc.WindUI then Misc.WindUI:Notify({ Title = "Anti-Stuck Triggered", Content = "Character released from stuck state!", Icon = "lucide:unlock" }) end
    end)
end
...
function Misc.StartAutoFarmAI()
    ...
    task.spawn(function()
        while task.wait(0.4) do
            ...
            if not Misc.Config.Current.AutoFarmBot then getgenv().CachedWaypoints = nil; getgenv().AIFinalTarget = nil; continue end
            pcall(function()
                ...
                if IsImmobilized() then getgenv().CachedWaypoints = nil; getgenv().AIFinalTarget = nil; return end
                    ...
                    if p ~= LocalPlayer and p.Character then
                        ...
                        if eRoot then
                            ...
                            if eTeam:find("killer") then
                                if dist < closestKillerDist then closestKillerDist = dist; killerRoot = eRoot end
                            else
                                ...
                                if (isKnocked or isInjured) and dist < shortestMateDist then shortestMateDist = dist; injuredTeammate = p.Character end
                            end
                        end
                    end
                end
                ...
                if Misc.ESP and Misc.ESP.CachedMapObjects and Misc.ESP.CachedMapObjects.Generators then
                    for _, gen in ipairs(Misc.ESP.CachedMapObjects.Generators) do
                        ...
                        if progress >= 100 then completedGens = completedGens + 1
                        else
                            local genPos = gen:GetPivot().Position
                            local dist = (genPos - myPos).Magnitude
                            if dist < shortestGenDist then shortestGenDist = dist; bestGenTarget = genPos end
                        end
                    end
                end
                local targetPos = nil
                ...
                if closestKillerDist <= 70 and killerRoot then
                    ...
                    local function checkSafeSpot(spot) local spotPos = spot:GetPivot().Position; local distFromKiller = (spotPos - killerPos).Magnitude; if distFromKiller > maxDistFromKiller then maxDistFromKiller = distFromKiller; bestEscapeTarget = spotPos end end
                    if Misc.ESP and Misc.ESP.CachedMapObjects then
                        if Misc.ESP.CachedMapObjects.Generators then for _, g in ipairs(Misc.ESP.CachedMapObjects.Generators) do checkSafeSpot(g) end end
                        if Misc.ESP.CachedMapObjects.Gates then for _, g in ipairs(Misc.ESP.CachedMapObjects.Gates) do checkSafeSpot(g) end end
                    end
                    if bestEscapeTarget then targetPos = bestEscapeTarget
                    else local runDir = (myPos - killerPos).Unit; targetPos = myPos + (runDir * 50) end
                    actionState = "Evading"
                elseif injuredTeammate then
                    ...
                    if shortestMateDist <= 12 then
                        if not Misc.SearchHealRemote then
                            local remotes = Misc.ReplicatedStorage:FindFirstChild("Remotes")
                            Misc.CachedHealEvent = remotes and (remotes:FindFirstChild("HealEvent", true) or remotes:FindFirstChild("RequestHeal", true) or remotes:FindFirstChild("ReviveEvent", true))
                            Misc.SearchHealRemote = true
                        end
                        if Misc.CachedHealEvent then
                            pcall(function() Misc.CachedHealEvent:FireServer(injuredTeammate, 100) end)
                            pcall(function() Misc.CachedHealEvent:FireServer(injuredTeammate, true) end)
                        end
                ...
                elseif completedGens >= 5 and Misc.ESP and Misc.ESP.CachedMapObjects and Misc.ESP.CachedMapObjects.Gates then
                    ...
                    for _, gate in ipairs(Misc.ESP.CachedMapObjects.Gates) do
                        ...
                        if dist < shortestGate then shortestGate = dist; targetPos = gatePos end
                    end
                ...
                if getgenv().LastAIState ~= actionState then
                    ...
                    if actionState ~= "Idle" and Misc.WindUI then
                        local notifIcons = { Evading = "lucide:footprints", Healing = "lucide:heart-handshake", Repairing = "lucide:wrench", Escaping = "lucide:door-open", Idle = "lucide:coffee" }
                        Misc.WindUI:Notify({ Title = "AI State: " .. string.upper(actionState), Content = "Switching AI priority to: " .. actionState, Icon = notifIcons[actionState] or "lucide:bot", Duration = 3 })
                    end
                end
                ...
                if targetPos then
                    ...
                    if (targetPos - lastTargetPos).Magnitude > 5 or (now - lastPathCalc > 1.5) then
                        ...
                        task.spawn(function()
                            pcall(function()
                                local path = Misc.PathfindingService:CreatePath({ AgentRadius = 2.5, AgentHeight = 5, AgentCanJump = true, WaypointSpacing = 4 })
                                path:ComputeAsync(myPos, targetPos)
                                if path.Status == Enum.PathStatus.Success then
                                    getgenv().CachedWaypoints = path:GetWaypoints()
                                    getgenv().CurrentWaypointIdx = 2
                                else getgenv().CachedWaypoints = nil end
                            end)
                        end)
                    end
                else
    ...
    task.spawn(function()
        while task.wait(0.05) do
            if not getgenv().PINATHUB_RUNNING then break end
            if not Misc.Config.Current.AutoFarmBot then continue end
            pcall(function()
                local myChar = LocalPlayer.Character
                local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
                local myHum = myChar and myChar:FindFirstChild("Humanoid")
                ...
                if waypoints and idx and idx <= #waypoints then
                    ...
                    if distToWaypoint < 4.5 then
                        ...
                        if getgenv().CurrentWaypointIdx <= #waypoints then nextPoint = waypoints[getgenv().CurrentWaypointIdx] end
                    end
                    if nextPoint then
                        myHum:MoveTo(nextPoint.Position)
                        if nextPoint.Action == Enum.PathWaypointAction.Jump then myHum.Jump = true end
                    end
                elseif getgenv().AIFinalTarget then myHum:MoveTo(getgenv().AIFinalTarget) end
...
function Misc.SetupNamecallHook()
    local oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if checkcaller() or method ~= "FireServer" or typeof(self) ~= "Instance" then return oldNamecall(self, ...) end
        local args = { ... }
                ...
                task.spawn(function()
                    ...
                    if hum and root then
                        ...
                        for _, track in ipairs(hum:GetPlayingAnimationTracks()) do
                            ...
                            if name:find("break") or name:find("generator") or name:find("kick") then pcall(function() track:Stop(0) end) end
                        end
                        hum:ChangeState(Enum.HumanoidStateType.Running)
                    end
                end)
        ...
        if Misc.Config.Current.AntiLogger and (n:find("log") or n:find("error") or n:find("report") or n:find("anticheat") or n:find("ban")) then return end
        if Misc.Config.Current.AntiFallDamage and (n:find("falldamage") or n:find("fall") or n:find("ragdollfall")) then return end
        ...
        if Misc.Config.Current.SilentAimPistol and method == "FireServer" and n:find("fire") then
            local team = LocalPlayer.Team
            ...
            if survivor then
                ...
                local firing = Misc.UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or (Misc.UserInputService.TouchEnabled and getgenv().isMobileFiring)
                ...
                if target and target.Parent then
                    ...
                    local ping = 0.08
                    pcall(function() ping = Misc.Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000 end)
                    ...
                    for i, v in ipairs(args) do
                        if typeof(v) == "Vector3" then args[i] = dir; break end
                    end
                    task.spawn(function() pcall(function() workspace.CurrentCamera.CFrame = CFrame.lookAt(workspace.CurrentCamera.CFrame.Position, predicted) end) end)
                    return oldNamecall(self, unpack(args))
                end
            end
        end
